...
- **PROC-03 `decisions[]` convention for inlined-abstraction deviations.** When a plan author makes a non-mechanical implementation choice the requirement text does NOT name — most commonly inlining an abstraction the requirement names (e.g. v0.1's POST-02 inlining of `ThresholdPolicy` into `LabelResolver.effectiveThreshold`) but also design-coupling decisions across multiple REQs (REF-02 + REF-04 paired in a single Magika.java commit because SC-6 is only testable when both exist), wire-format spike results, ArchUnit-rule carve-outs, or Spotless / Rule-1 auto-fixes — that decision is recorded as a `decisions[]` entry in the plan SUMMARY frontmatter at SUMMARY-write time. Each entry is a YAML mapping with three required string fields: `topic` (free-text — conventionally references a REQ-ID or a design topic; the lint does NOT enforce a regex on `topic` per D-11 because design-coupling topics like "REF-02 + REF-04 pairing" don't match a single REQ-ID shape), `decision` (what was chosen; may use YAML block scalar `|` for multi-line content), `rationale` (why; review-time judgment, no length enforcement). The convention closes the v0.1 retro lesson "POST-02 design deviation surfaced late" — when an audit asks "why is this not a separate class?", the answer lives in machine-readable frontmatter rather than buried prose. **Canonical worked example:** `.planning/phases/02-tech-debt-parity-coverage-expansion/02-01-debt-closure-SUMMARY.md` `decisions[]` entry with `topic: "POST-02 inlining"` documents the v0.1 inlining + DEBT-03 extraction closure as the prototype shape future planners copy. **String-shaped entries are NOT permitted** (per B-03): plain strings under `decisions:` will fail Jackson `List<DecisionEntry>` deserialization in PROC-01's lint; mixed string + object lists fail the same way. If a plan has no deviations to record, OMIT the `decisions:` field entirely (B-11 — the lint treats absence as empty list).
